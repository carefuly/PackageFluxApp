...
type File struct {
	...
	BaseDir string `gorm:"type:varchar(255);comment:项目根目录" json:"base_dir"`    // 项目根目录
	FileDir string `gorm:"type:text;comment:文件绝对路径" json:"file_dir"`           // 文件绝对路径
	...
	UserID  uint   `gorm:"type:int;comment:用户ID" json:"user_id"`               // 用户ID
	User    User   `gorm:"foreignKey:UserID;constraint:OnDelete:CASCADE;"`     // 用户
}
